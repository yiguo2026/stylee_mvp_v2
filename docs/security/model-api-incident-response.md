...
The App and GitHub Pages build must never receive DeepSeek, DashScope, Ark, or Supabase service-role credentials. They are server-only model-service environment variables.
...
5. If a Supabase service-role key ever entered a Web build, create a new `sb_secret_...` key and store it only as model-service `SUPABASE_SECRET_KEY`; after migration, disable the compromised legacy key.
