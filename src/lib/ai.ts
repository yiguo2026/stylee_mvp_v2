...
规则：
1. 只生成1套搭配方案，选择最合适的组合
2. 搭配必须包含上装和下装（或连体装），可选配外套/鞋履/包袋/帽巾/配饰，3-5件单品
3. 优先使用衣橱中已有的单品，通过ID引用（owned_item_ids）
4. recommended_items 中可推荐1-2件衣橱中没有的单品作为搭配补充
5. recommended_items 中每件需要 name（名称）、category（分类：上装/下装/连体装/外套/鞋履/包袋/帽巾/配饰）、color（颜色）、description（简短描述，如"百搭内搭·四季"）
6. 颜色搭配协调，风格统一
7. 考虑天气和场合
8. comment 是搭配理由，2-3句话说明为什么这些单品搭配在一起好看
...
重要：只返回1套最佳方案。如果衣橱单品不够完整搭配，多推荐recommended_items来补足。`;
