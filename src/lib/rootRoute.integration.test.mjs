...
import { completeBootstrap, deferGender, deferPreferenceLoad, emitAuth, finishPreferenceLoad,
  observations, resetHarness, setPath, syntheticSession,
  webAuthCoordinator } from './test-fixtures/rootRoute/platform.mjs';

globalThis.IS_REACT_ACT_ENVIRONMENT = true;

test('INITIAL_SESSION starts profile and preference reads once without blocking navigation', async () => {
  resetHarness('/wardrobe/private-item');
  deferPreferenceLoad('A');
  let root;
  await act(async () => { root = create(React.createElement(RootLayout)); completeBootstrap(syntheticSession()); });
  try {
    assert.equal(observations.profileReads, 1);
    assert.equal(observations.preferenceLoads, 1);
    assert.deepEqual(observations.routes, ['/(tabs)']);
    assert.equal(observations.preferencePublications, 0);
    await act(async () => { finishPreferenceLoad(); });
    assert.equal(observations.preferenceAccountId, 'A');
  } finally { await act(async () => root.unmount()); }
});

test('same-account refresh and update signals do not duplicate preference load', async () => {
  resetHarness('/wardrobe/private-item');
  let root;
  await act(async () => { root = create(React.createElement(RootLayout)); completeBootstrap(syntheticSession()); });
  try {
    await act(async () => { emitAuth('TOKEN_REFRESHED', { ...syntheticSession(), refresh_token: 'synthetic-new' }); });
    await act(async () => { emitAuth('USER_UPDATED', { ...syntheticSession(), refresh_token: 'synthetic-new' }); });
    assert.equal(observations.preferenceLoads, 1);
    assert.equal(observations.profileReads, 1);
  } finally { await act(async () => root.unmount()); }
});

test('password recovery starts the account preference load', async () => {
  resetHarness('/profile/change-password');
  let root;
  await act(async () => { root = create(React.createElement(RootLayout)); });
  try {
    await act(async () => { emitAuth('PASSWORD_RECOVERY', syntheticSession()); });
    assert.equal(observations.preferenceLoads, 1);
    assert.equal(observations.profileReads, 1);
    assert.deepEqual(observations.routes, ['/profile/change-password']);
  } finally { await act(async () => root.unmount()); }
});

test('A to B resets preference state before B load and late A completion stays inert', async () => {
  resetHarness('/wardrobe/private-item');
  deferPreferenceLoad('A');
  let root;
  await act(async () => { root = create(React.createElement(RootLayout)); completeBootstrap(syntheticSession()); });
  try {
    observations.accountEvents.length = 0;
    await act(async () => { emitAuth('SIGNED_IN', syntheticSession('B')); });
    assert.deepEqual(observations.accountEvents, ['preference-reset', 'preference-load:B']);
    assert.equal(observations.preferenceAccountId, 'B');
    await act(async () => { finishPreferenceLoad(); });
    assert.equal(observations.preferenceAccountId, 'B');
    assert.equal(observations.preferencePublications, 1);
  } finally { await act(async () => root.unmount()); }
});
...
test('anonymous bootstrap waits for the root route to mount before navigation', async () => {
  ...
  try {
    ...
    assert.equal(observations.preferenceLoads, 0);
    assert.equal(observations.navigationWithoutRoute, 0);
  } finally { await act(async () => root.unmount()); }
});
